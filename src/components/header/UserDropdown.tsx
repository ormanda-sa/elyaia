...
export default function UserDropdown() {
  ...
  useEffect(() => {
    ...
    async function loadProfile() {
      try {
        ...
        if (!res.ok) return;
        if (!cancelled && json?.basic) {
          setBasic({
            fullName: json.basic.fullName,
            role: json.basic.role,
            email: json.basic.email,
            avatarUrl: json.basic.avatarUrl,
          });
        }
      } catch {
        // نتجاهل الخطأ هنا
  ...
  const displayName = basic?.fullName || "صاحب المتجر";
  const displayRole = basic?.role || "";
  const displayEmail = basic?.email || "";
