"use client";

import * as React from "react";
import * as XLSX from "xlsx";
import {
  Dialog,
  DialogContent,
  DialogHeader,
  DialogTitle,
  DialogTrigger,
} from "@/components/ui/dialog";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";

type RawYearKeywordItem = {
  name_ar: string;
  sort_order?: number | string | null;
};

type Props = {
  yearId: number | null;
  yearLabel?: string | null; // للعرض فقط
  disabled?: boolean;
  onImported: () => Promise<void> | void;
};

export function YearKeywordsBulkImportDialog({
  yearId,
  yearLabel,
  disabled,
  onImported,
}: Props) {
  const [open, setOpen] = React.useState(false);
  const [jsonText, setJsonText] = React.useState("");
  const [file, setFile] = React.useState<File | null>(null);
  const [parsingError, setParsingError] = React.useState<string | null>(null);
  const [importError, setImportError] = React.useState<string | null>(null);
  const [importSuccess, setImportSuccess] = React.useState<string | null>(null);
  const [isImporting, setIsImporting] = React.useState(false);

  function resetState() {
    setJsonText("");
    setFile(null);
    setParsingError(null);
    setImportError(null);
    setImportSuccess(null);
  }

  function handleDownloadTemplate() {
    const rows = [
      ["name_ar", "sort_order"],
      ["مكينة", 1],
      ["قير", 2],
      ["رديتر", 3],
    ];

    const worksheet = XLSX.utils.aoa_to_sheet(rows);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "year_keywords");

    const wbout = XLSX.write(workbook, { bookType: "xlsx", type: "array" });
    const blob = new Blob([wbout], {
      type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
    });

    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.download = "year-keywords-template.xlsx";
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
  }

  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    setParsingError(null);
    setImportError(null);
    setImportSuccess(null);

    const f = e.target.files?.[0];
    if (!f) {
      setFile(null);
      return;
    }
    setFile(f);

    try {
      const arrayBuffer = await f.arrayBuffer();
      const workbook = XLSX.read(arrayBuffer, { type: "array" });

      const sheetName = workbook.SheetNames[0];
      const worksheet = workbook.Sheets[sheetName];

      const rawRows: any[] = XLSX.utils.sheet_to_json(worksheet, {
        defval: "",
      });

      if (rawRows.length === 0) {
        setParsingError("الملف لا يحتوي على بيانات.");
        return;
      }

      const items: RawYearKeywordItem[] = rawRows.map((row, idx) => {
        const name_ar = (row.name_ar || row["name_ar"] || "")
          .toString()
          .trim();

        if (!name_ar) {
          throw new Error(
            `السطر رقم ${idx + 2} لا يحتوي على name_ar (نص الكلمة).`,
          );
        }

        const sort_orderRaw =
          row.sort_order !== undefined && row.sort_order !== null
            ? String(row.sort_order).trim()
            : "";

        return {
          name_ar,
          sort_order: sort_orderRaw ? Number(sort_orderRaw) : null,
        };
      });

      setJsonText(JSON.stringify(items, null, 2));
      setParsingError(null);
    } catch (err: any) {
      console.error("parse year_keywords excel error:", err);
      setParsingError(
        err?.message || "تعذّر قراءة ملف Excel، تأكد من استخدام القالب.",
      );
    }
  }

  function parseJsonInput(): RawYearKeywordItem[] {
    setParsingError(null);

    if (!jsonText.trim()) {
      throw new Error("الرجاء إدخال بيانات JSON أو اختيار ملف.");
    }

    let parsed: any;
    try {
      parsed = JSON.parse(jsonText);
    } catch {
      throw new Error("صيغة JSON غير صحيحة، تحقق من الفاصلات والأقواس.");
    }

    if (!Array.isArray(parsed)) {
      throw new Error("توقّعنا مصفوفة JSON: [ { ... }, { ... } ]");
    }

    const items: RawYearKeywordItem[] = parsed.map((row: any, idx: number) => {
      const name_ar = (row.name_ar || "").toString().trim();
      if (!name_ar) {
        throw new Error(`العنصر رقم ${idx + 1} لا يحتوي على name_ar.`);
      }
      return {
        name_ar,
        sort_order:
          row.sort_order !== undefined && row.sort_order !== null
            ? Number(row.sort_order)
            : null,
      };
    });

    return items;
  }

  async function handleImport() {
    if (!yearId) {
      setImportError("اختر السنة أولاً قبل الاستيراد.");
      return;
    }

    setImportError(null);
    setImportSuccess(null);
    setParsingError(null);
    setIsImporting(true);

    try {
      const items = parseJsonInput();
      if (!items.length) throw new Error("لا توجد عناصر بعد التحليل.");

      // ✅ Bulk API (أسرع + أقل طلبات)
      const res = await fetch("/api/dashboard/year-keywords/bulk", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ year_id: yearId, items }),
      });

      const data = await res.json().catch(() => ({} as any));

      if (!res.ok) {
        throw new Error(data?.error || "فشل الاستيراد");
      }

      const successCount = data?.successCount ?? 0;
      const failCount = data?.failCount ?? 0;

      setImportSuccess(
        `تم استيراد ${successCount} كلمة بنجاح، وفشل ${failCount} (إن وجد).`,
      );
      await onImported();
    } catch (err: any) {
      setParsingError(err?.message || "خطأ أثناء تحليل/استيراد البيانات.");
    } finally {
      setIsImporting(false);
    }
  }

  return (
    <Dialog
      open={open}
      onOpenChange={(v) => {
        setOpen(v);
        if (!v) resetState();
      }}
    >
      <DialogTrigger asChild>
        <Button
          type="button"
          size="sm"
          className="rounded-full"
          variant="outline"
          disabled={disabled || !yearId}
        >
          استيراد كلمات السنة (Excel)
        </Button>
      </DialogTrigger>

      <DialogContent dir="rtl" className="sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle className="text-right text-sm">
            استيراد كلمات السنة {yearLabel ? `— ${yearLabel}` : ""}
          </DialogTitle>
        </DialogHeader>

        <div className="space-y-3 text-[11px] text-slate-700">
          {!yearId ? (
            <p className="rounded-xl border border-yellow-200 bg-yellow-50 px-3 py-2 text-[11px] text-yellow-800">
              اختر السنة أولاً قبل الاستيراد.
            </p>
          ) : null}

          <div className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-[10px] text-amber-800">
            قالب Excel يحتوي على الأعمدة:
            <span className="font-mono"> name_ar, sort_order</span>.
          </div>

          <div className="space-y-1">
            <label className="block text-right text-[11px] font-medium">
              ملف Excel (.xlsx)
            </label>
            <div className="flex items-center gap-2">
              <Input
                type="file"
                accept=".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                className="h-8 text-[11px]"
                onChange={handleFileChange}
              />
              <Button
                type="button"
                size="sm"
                variant="outline"
                className="rounded-full text-[11px]"
                onClick={handleDownloadTemplate}
              >
                تحميل قالب Excel
              </Button>
            </div>
            <p className="text-[10px] text-slate-500">
              افتح القالب، أضف كل كلمة في سطر مع ترتيبها، ثم احفظ وارفع الملف.
            </p>
          </div>

          <div className="space-y-1">
            <label className="block text-right text-[11px] font-medium">
              JSON (اختياري)
            </label>
            <textarea
              value={jsonText}
              onChange={(e) => setJsonText(e.target.value)}
              rows={8}
              className="w-full rounded-md border border-slate-200 bg-white px-2 py-1 text-[11px] font-mono focus:outline-none focus:ring-2 focus:ring-slate-400"
              placeholder='[{"name_ar":"مكينة","sort_order":1}]'
            />
          </div>

          {parsingError && (
            <p className="rounded-lg border border-red-100 bg-red-50 px-3 py-2 text-[11px] text-red-600">
              {parsingError}
            </p>
          )}

          {importError && (
            <p className="rounded-lg border border-red-100 bg-red-50 px-3 py-2 text-[11px] text-red-600">
              {importError}
            </p>
          )}

          {importSuccess && (
            <p className="rounded-lg border border-emerald-100 bg-emerald-50 px-3 py-2 text-[11px] text-emerald-700">
              {importSuccess}
            </p>
          )}

          <div className="flex justify-between gap-2 pt-1">
            <Button
              type="button"
              variant="outline"
              size="sm"
              className="rounded-full"
              onClick={resetState}
              disabled={isImporting}
            >
              مسح الحقول
            </Button>
            <Button
              type="button"
              size="sm"
              className="rounded-full"
              onClick={handleImport}
              disabled={isImporting || !yearId}
            >
              {isImporting ? "جاري الاستيراد..." : "بدء الاستيراد"}
            </Button>
          </div>
        </div>
      </DialogContent>
    </Dialog>
  );
}