// FILE: src/app/widgets.allPageT2/route.ts
...
(function () {
  try {
    ...
    function parseKeywordLabels(value) {
      var keyword = String(value || "").trim();
      if (!keyword) return [];

      if (keyword.indexOf("||") > -1) {
        return keyword.split("||").filter(function (x) {
          return x && x.trim();
        });
      }
    ...
    function parseCurrentUrlFilters() {
      ...
      try {
        ...
        var seoMatch = pathname.match(/^\\/(.+)\\/c([0-9]+)$/i);
        if (seoMatch) {
          result.pathSlug = seoMatch[1];
          result.modelCategoryId = seoMatch[2];
        }

        var oldCategoryMatch = pathname.match(/^\\/category\\/([^/?#]+)/i);
        if (oldCategoryMatch && !result.pathSlug) {
          result.pathSlug = decodeURIComponent(oldCategoryMatch[1]);
        }
        ...
        result.keywordLabels = parseKeywordLabels(keyword);
      } catch (e) {}
    ...
    function buildAllPageFilter() {
      (async function () {
        ...
        wrap.className = "zzz widgets-filter-hero-wrap";
      ...
      <div class="X1">\\
        ...
        wrap.querySelectorAll(".hero-filters-form select").forEach(function (el, idx) {
          if (steps[idx]) {
        ...
        filterBtn.addEventListener("click", async function () {
          ...
          try {
            ...
            console.error("[widgets.allPage.js] search click error:", err);
        ...
        async function prefillFromUrl() {
          ...
          var preBrand = null;
          var preModelGlobal = null;

          try {
            var snap = await ensureSnapshot(storeId);
            var allModels = (snap && snap.models) || [];

            if (info.modelCategoryId || info.pathSlug || info.sCategory) {
              preModelGlobal =
                allModels.find(function (m) {
                  var modelCategoryId =
                    getRowValue(m, [
                      "category.id",
                      "ccategory.id",
                  ...
                  return (
                    (info.modelCategoryId &&
                      String(modelCategoryId) === String(info.modelCategoryId)) ||
                    (info.sCategory &&
                      String(modelCategoryId) === String(info.sCategory)) ||
                    (info.pathSlug &&
                      String(m.slug || "") === String(info.pathSlug))
                  );
                }) || null;
            }
              ...
              preBrand =
                brands.find(function (b) {
                  return (
                    ...
                    String(b.salla_brand_id || "") === String(info.sCompany) ||
                    String(b.id) === String(info.sCompany)
                  );
                }) || null;
            }

            if (!preBrand && preModelGlobal) {
              preBrand =
                brands.find(function (b) {
                  return String(b.id) === String(preModelGlobal.brand_id);
                }) || null;
            }
            ...
            if (preModelGlobal) {
              preModel =
                models.find(function (m) {
                  return String(m.id) === String(preModelGlobal.id);
                }) || null;
            }

            if (!preModel && (info.modelCategoryId || info.sCategory)) {
              preModel =
                models.find(function (m) {
                  var modelCategoryId =
                    getRowValue(m, [
            ...
            if (info.sYear) {
              preYear =
                years.find(function (y) {
                  ...
                  return (
                    String(yearCategoryId) === String(info.sYear) ||
    ...
    function loadAllPageWidget() {
      try {
        ...
        var isNewCategorySeo = /^\\/.+\\/c[0-9]+$/i.test(pl);
